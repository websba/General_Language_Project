...
class LanguagePracticeApp:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.words: list[str] = []
        self.zh_voice_var = tk.BooleanVar(value=False)
        ...
        self.last_word_index: int | None = None
    ...
    def _build_layout(self) -> None:
        ...
        top_opts = tk.Frame(self.root)
        top_opts.pack(fill=tk.X, padx=12, pady=(8, 0))
        self.zh_voice_check = tk.Checkbutton(
            top_opts,
            text="啟動中文聲音",
            variable=self.zh_voice_var,
            anchor="w",
        )
        self.zh_voice_check.pack(side=tk.LEFT)
        self.auto_play_check = tk.Checkbutton(
            top_opts,
            text="自動播放",
            variable=self.auto_play_var,
            anchor="w",
        )
        self.auto_play_check.pack(side=tk.LEFT, padx=(16, 0))
        ...
        status_probe = tk.Label(words_box)
        status_bg = str(status_probe.cget("background"))
        status_fg = str(status_probe.cget("foreground"))
        ...
        self.status_text.pack(side=tk.BOTTOM, fill=tk.X, pady=(8, 0))
        self.status_text.bind("<Control-a>", self._select_all_status)
        self.status_text.bind("<Control-A>", self._select_all_status)
        self.set_status("就緒")
    ...
    def _select_all_status(self, _event: tk.Event[tk.Misc]) -> str:
        self.status_text.tag_add("sel", "1.0", "end-1c")
        self.status_text.mark_set("insert", "1.0")
    ...
    def _rebuild_word_buttons(self) -> None:
        ...
        for index, word in enumerate(self.words):
            button = tk.Button(
                ...
                text=word,
                justify="center",
                command=lambda w=word, i=index: self.on_word_click(w, i),
            )
    ...
    def on_word_click(self, word: str, index: int) -> None:
        self.last_word = word
        self.last_word_index = index
        self._play_word(word)

    def _sync_last_word_index(self) -> None:
        if self.last_word is None:
            self.last_word_index = None
            return
        index = self.last_word_index
        if (
            index is not None
            and 0 <= index < len(self.words)
            and self.words[index] == self.last_word
        ):
            return
        try:
            self.last_word_index = self.words.index(self.last_word)
        except ValueError:
            self.last_word = None
            self.last_word_index = None

    def _maybe_auto_play_next(self, current_index: int | None) -> None:
        if not self.auto_play_var.get() or current_index is None:
            return
        if current_index < 0 or current_index >= len(self.words):
            return
        if self.words[current_index] != self.last_word:
            return
        next_index = current_index + 1
        if next_index >= len(self.words):
            self.set_status("自動播放完成")
            return
        self.on_word_click(self.words[next_index], next_index)
    ...
    def _play_word(self, word: str) -> None:
        ...
        speak_zh = bool(self.zh_voice_var.get())
        play_index = self.last_word_index
        thread = threading.Thread(
            target=self._speak_worker,
            args=(word, lang_name, lang_code, speak_zh, play_index),
            daemon=True,
        )
        thread.start()

    def _ensure_speech_file(self, text: str, lang_code: str, dest: Path) -> None:
        ...
        asyncio.run(generate_speech(text, resolve_voice(lang_code), tmp_path))
        tmp_path.replace(dest)

    def _play_mp3(self, dest: Path) -> None:
    ...
    def _speak_worker(
        ...
        zh_dest = cache_path_for("zh-tw", word)
        ...
        try:
            ...
            if speak_zh and (not zh_dest.exists() or zh_dest.stat().st_size == 0):
                status_cb(f"產生中文語音：{word}")
                self._ensure_speech_file(word, "zh-tw", zh_dest)
            if not dest.exists() or dest.stat().st_size == 0:
                status_cb(f"產生語音：{triple}")
                self._ensure_speech_file(translated, lang_code, dest)

            status_cb(f"播放中：{triple}")
            with self._play_lock:
                if speak_zh:
                    self._play_mp3(zh_dest)
                    time.sleep(0)
                self._play_mp3(dest)
            status_cb(f"播放完成：{triple}")
            played_ok = True
        except Exception as exc:  # noqa: BLE001 — 顯示給使用者
            for path in (dest, zh_dest):
            ...
            self._busy = False
            if played_ok:
                self.root.after(0, lambda i=play_index: self._maybe_auto_play_next(i))
