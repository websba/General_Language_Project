...
class LanguagePracticeApp:
    ...
    def _rebuild_language_radios(self, prev_lang: str) -> None:
        ...
        for index, (name, _code) in enumerate(self.languages):
            ...
            radio.grid(row=index // 5, column=index % 5, sticky="w", padx=6, pady=2)
    ...
    def _rebuild_word_buttons(self) -> None:
        ...
        for col in range(columns):
            self.words_frame.grid_columnconfigure(col, weight=1)
    ...
    def _speak_worker(self, word: str, lang_name: str, lang_code: str) -> None:
        ...
        try:
            if not dest.exists() or dest.stat().st_size == 0:
                ...
                translated = GoogleTranslator(source="zh-TW", target=lang_code).translate(
                    word
                )
                if not translated:
                    raise RuntimeError("翻譯結果是空的")
                self.root.after(
                    0,
            ...
            self.root.after(0, lambda: self.set_status(f"播放失敗：{exc}"))
        finally:
            self._busy = False
